...
- 行情仓：`backend/data/market.db`（增量同步：已是 T-1 跳过；缺几日只补几日；`force=true` 强制全量）
...
`base_amount` 为**月定投总预算**；可在设置中选每日/每周/每月执行日，系统按交易日历折算本期金额。硬否决与倍数按标的 profile 分化；止盈为估值武装 + 峰值回撤。信号使用 **T-1** 收盘数据。  
